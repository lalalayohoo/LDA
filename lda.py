...
from drawperplexity import graph_draw
import argparse
...
listmin = args.min
listmax = args.max
listinterrval = args.interval
poolnum = args.pool
choose = args.trainmode
iters_num = args.iters
twords_num = args.twordnum
topics_num = args.topicnum
...
class LdaBase(CorpusSet):
    ...
    def save_model(self):
        ...
        self.save_tag(os.path.join(self.dir_path, "%s.%s" % (name_predix, "theta")))
        return
...
class LdaModel(LdaBase):
    ...
    def init_train_model(self, dir_path, model_name, current_iter, iters_num=None, topics_num=10, twords_num=20,
        ...
        :param twords_num: 可以为整数值,即文件中每个主题展示的twords数量
        """
        if current_iter == 0:
    ...
    def begin_gibbs_sampling_train(self, is_calculate_preplexity=True):
        ...
        :param is_calculate_preplexity: 设置是否计算perplexity，True或None
        """
        # Gibbs抽样
...
def lda(i):
    ...
    :key: 当加载已有模型时,只需要dir_path, model_name, current_iter（不等于0）, iters_num, twords_num即可
    :param iters_num: 可以为整数值或者“auto”
    :param topics_num: 主题数K，可以为整数值
    :param twords_num: 可以为整数值,即文件中每个主题展示的twords数量
    """
    logging.basicConfig(level=logging.DEBUG, format="%(asctime)s\t%(levelname)s\t%(message)s")
    print(i)
    model = LdaModel()
    model.init_train_model("data/", "model", current_iter=0, iters_num=iters_num, topics_num=i, twords_num=twords_num,
...
if __name__ == "__main__":
    """
    :param choose: 可以为"perplexity"或者"train"，分别对应计算困惑度模式训练或直接训练

    """

    if choose == "perplexity":
        time1 = time.time()

        # 生成主题列表
        list = []
        for x in range(listmin, listmax + listinterrval, listinterrval):
            list.append(x)
        print(list)
        ...
        # 绘制困惑度折线图
        perplexity = [i[-1] for i in res]
        print(perplexity)
        with open('./data/perplexity_topic_lda.txt', 'w') as perplexity_file:
            perplexity_file.write(str(perplexity))
        graph_draw(list, perplexity)
        ...
        model = LdaModel()
        model.init_train_model("data/", "model", current_iter=0, iters_num=iters_num, topics_num=topics_num, twords_num=twords_num,
                               data_file="data/corpus.txt")
        model.begin_gibbs_sampling_train()
        pp = model.ppdraw
        with open('./data/perplexity_iter_lda.txt', 'w') as perplexity_file:
            perplexity_file.write(str(pp))
